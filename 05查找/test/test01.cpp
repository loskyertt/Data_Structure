...
int main() {
  ...
  // 查找节点
  if (bst::is_existed_recur(root, 4)) {
    std::cout << "节点 4 存在！\n\n";
  } else {
    std::cout << "节点 4 不存在！\n\n";
  }
  ...
  // 插入节点 7
  bst::insert_recur(root, 7);
  std::cout << "插入节点 7：";
  bst::in_order(root);
  std::cout << "\n\n";

  // 插入节点 13
  bst::insert_recur(root, 13);
  std::cout << "插入节点 13：";
  bst::in_order(root);
  std::cout << "\n\n";

  // 删除节点 2（度为 0）
  bst::remove_recur(root, 2);
  std::cout << "删除节点 2：";
  bst::in_order(root);
  std::cout << "\n\n";

  // 删除节点 6（度为 1，右子节点是 7）
  bst::remove_recur(root, 6);
  std::cout << "删除节点 6：";
  bst::in_order(root);
  std::cout << "\n\n";

  // 删除节点 12（度为 2，左子节点是 10，右子节点是 13）
  bst::remove_recur(root, 12);
  std::cout << "删除节点 12：";
  bst::in_order(root);
  std::cout << "\n\n";

  // 删除根节点 8（度为 2，左子节点是 4，右子节点是12）
  bst::remove_recur(root, 8);
  std::cout << "删除节点 8：";
  bst::in_order(root);
  std::cout << "\n\n";
